...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "OptimusTemp"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
try {
const listaDirectorios = readdirSync('./OptimusTemp/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./OptimusTemp/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./OptimusTemp/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./OptimusTemp/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./Optimus/', './OptimusTemp/']
